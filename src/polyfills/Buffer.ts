/*
	This class is a zero-dependency polyfill largely based on https://github.com/feross/buffer, but some features have
	been dropped in order to achieve this, such as support for floats, doubles, and base64 encoding. We should make
	sure to minify and strip comments from this file during release or distribution. It's kinda big. ^^
*/
...
/**
 * `Buffer` objects are used to represent a fixed-length sequence of bytes. They are subclasses of the `Uint8Array`
 * class and extend it with methods that cover additional use cases. They are also backwards-compatible, so anything
 * that accepts a `Uint8Array` can accept a `Buffer` as well.
 *
 * If the current environment offers its own native `Buffer` in the global scope, such as with Node.js, then that class
 * will be exported. However, in environments which do not offer a native `Buffer` class, this polyfill is exported.
 *
 * You can create instances with the `Buffer.from()` method. Calling the constructor directly is deprecated.
 *
 * ```ts
 * // Creates a zero-filled buffer with 10 bytes.
 * const buf1 = Buffer.alloc(10);
 *
 * // Creates a buffer with 10 bytes all filled with the value 1.
 ...
 *
 * // Creates a buffer containing the latin1 bytes [0x74, 0xe9, 0x73, 0x74].
...
class Buffer extends Uint8Array {
	...
	/**
	 * This will be `true` if the exported `Buffer` class is a polyfill, or `undefined` if the current environment
	 * offers its own polyfill.
	 */
	public static isPolyfill = true;
	 ...
	 * @param length
	 */
	protected constructor(value: string | number | Uint8Array | ArrayBuffer | number[] | Buffer, encodingOrOffset?: Encoding | number, length?: number) {
		// Handle numbers
		...
		else if (Buffer._isInstance(value, ArrayBuffer) || (value && Buffer._isInstance((value as any).buffer, ArrayBuffer))) {
	...
	/**
	 * The same as `buf.toString()`.
	 *
	 * @param encoding
	 */
	public toLocaleString(encoding?: Encoding) {
	 ...
	 */
	public compare(otherBuffer: Uint8Array, targetStart?: number, targetEnd?: number, sourceStart?: number, sourceEnd?: number): number {
    ...
    subarray(begin?: number, end?: number): Buffer {
		return super.subarray(begin, end) as Buffer;
	}
	...
	public fill(value: any, offset?: number, end?: number, encoding?: Encoding): this {
		...
		if (typeof value === 'number') {
			...
			if (len === 0) {
				throw new TypeError('The value "' + value +
	...
	private _bidirectionalIndexOf(buffer: Buffer, val: string | number | Buffer, byteOffset?: number, encoding?: Encoding, dir?: boolean) {
	 ...
	 * @param encoding
	 */
	public static from(a: string | number | Uint8Array | number[] | Buffer | ArrayBuffer, b?: Encoding | number, c?: number): Buffer {
		if (Buffer.isBuffer(a) && b === undefined && c === undefined) {
			return a;
		}
	...
	private static _blitBuffer(src: number[], dst: Buffer, offset: number, length: number) {
		let i: number;
 ...
 */
export type Encoding = 'ascii' | 'utf8' | 'utf16le' | 'ucs2' | 'binary' | 'hex' | 'latin1';
